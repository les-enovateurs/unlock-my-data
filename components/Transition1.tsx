  ...
  return (
    ...
    >
      ...
      <div className="w-full h-[20vh] flex items-end justify-end z-30">
        <div className="h-[20vh] bg-texteBlack w-1/2 text-white z-30">
          AAAAA
  ...
  useEffect(() => {
    ...
    while (attempts < maxAttempts) {
      ...
      const checkPoints = [
        ...
        { x: x + width * (1 - overflowTolerance), y: y + height * overflowTolerance },
        { x: x + width * overflowTolerance, y: y + height * (1 - overflowTolerance) },
        { x: x + width * (1 - overflowTolerance), y: y + height * (1 - overflowTolerance) },
      ];
      ...
      if (isPartiallyInside && !isOverlapping({ x, y, width, height }, placedRects)) {
        ...
        const element = (
          ...
          >
            <div ref={el => elementRefs.current[index] = el} className="relative w-full h-full">
    ...
    const optimizeShapes = () => {
      ...
      while (madeChanges) {
        ...
        placedRects.forEach((rect, index) => {
          ...
          const checkPoints = [
            { x: newRect.x + newRect.width * overflowTolerance, y: newRect.y + newRect.height * overflowTolerance },
            { x: newRect.x + newRect.width * (1 - overflowTolerance), y: newRect.y + newRect.height * overflowTolerance },
            { x: newRect.x + newRect.width * overflowTolerance, y: newRect.y + newRect.height * (1 - overflowTolerance) },
            { x: newRect.x + newRect.width * (1 - overflowTolerance), y: newRect.y + newRect.height * (1 - overflowTolerance) }
          ];
          ...
          const isValidGrowth = 
            ...
            !placedRects.some((otherRect, otherIndex) => 
