...
import { DIGITAL_CLEAN_UP_SUITES, ServiceSuite } from "@/constants/digitalCleanUp";
...
export default function ServiceSelection({
            ...
            .then((data: Service[]) => {
                if (mounted) {
                    ...
                    if (!hasInitDefaults.current) {
                        try {
                            const cached = localStorage.getItem("digitalCleanUpSelected");
                            if (cached) {
    ...
    useEffect(() => {
        if (hasInitDefaults.current) {
            localStorage.setItem("digitalCleanUpSelected", JSON.stringify(selectedServices));
        }
    }, [selectedServices]);
    ...
    const toggleSuite = (suite: ServiceSuite) => {
        const suiteCurrentChildren = suite.children;
    ...
    const isSuiteFullySelected = (suite: ServiceSuite) => suite.children.length > 0 && suite.children.every(slug => selectedServices.includes(slug));
    const isSuitePartiallySelected = (suite: ServiceSuite) => {
        const selectedCount = suite.children.filter(slug => selectedServices.includes(slug)).length;
        return selectedCount > 0 && selectedCount < suite.children.length;
    };
                    ...
                    {DIGITAL_CLEAN_UP_SUITES.map((suite) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <p className="text-sm opacity-60 mt-1 line-clamp-1">{suite.children.length} {t.t('services') || 'services'}</p>
                            </button>
                        );
                    })}
                </div>
