...
export const metadata: Metadata = {
  ...
  description: 'Informations détaillées sur l\'entreprise et ses pratiques de gestion des données',
};

export async function generateStaticParams() {
  try {
    // Lire le fichier JSON depuis le bon chemin
    const filePath = path.join(process.cwd(), "public/data/manual-entreprise.json");
    const jsonData = fs.readFileSync(filePath, "utf8");
    const entreprises: EntrepriseData[] = JSON.parse(jsonData);

    // Générer les paramètres pour chaque entreprise
    const params = entreprises
      .filter(entreprise => entreprise.Nom && entreprise.Nom.trim() !== "")
      .map((entreprise) => ({
